...
struct DwarvenMiner: SUI<unsigned, true, 3> {

  DwarvenMiner():SUI(Serial,Serial){}
  ...
  bool cliValidStation(unsigned param, const unsigned char key) {
    ...
    cout("Invalid station : ", param, ", valid values are [0..", numStations - 1, "] for command ", char(key));
    return false;
  }
  ...
  void handleKey(const unsigned char key, bool wasUpper) {
    Serial.print(": ");
    unsigned param = cli[0];  
    switch (key) {            //still available:aeh;[]
      ...
      case 'i':
        ...
        break;
      case 'q':
        for (unsigned i = 0; i < numRelays; ++i) {//in order for sanity's sake
      ...
      case 's':  // simulate a lever solution
        if (boss && cliValidStation(param, key)) {
          ...
          Serial.printf("Lever[%d] latch triggered, reports: %x, %u keys now activated\n", param, boss->lever[param], after);
        }
        break;
      case 't':
        if (boss) {
  ...
  void loop() {
    ...
    SUI::loop();  // process incoming keystrokes
    // time paced logic
...
void setup() {
    ...
    dm.cout.stifled = !debugging;  // opposite sense of following bug flags
    TRACE = debugging;
    BUG3 = debugging;
    BUG2 = debugging;
